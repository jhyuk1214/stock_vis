...
class ChartVisualizer:
    ...
    def create_chart(self, data, ma_200w, zones, current_zone, ticker):
        ...
        ax.plot(dates, prices, color='black', linewidth=1.5, label=f'{ticker} 주가')
        ax.plot(dates, ma_200w, color='white', linewidth=2, label='200주 이동평균선')
        
        self._add_zone_backgrounds(ax, dates, zones)
        
        ax.set_yscale('log')
        ax.set_title(f'{ticker} 주식 분석 차트 (현재 구간: {self._get_zone_korean(current_zone)})', 
                    fontsize=16, fontweight='bold')
        ax.set_xlabel('날짜', fontsize=12)
        ax.set_ylabel('가격 (로그 스케일)', fontsize=12)
    ...
    def _add_zone_backgrounds(self, ax, dates, zones):
        ...
        for zone_name in zone_order:
            ...
            rect = patches.Rectangle(
                ...
                facecolor=color,
                alpha=0.2,
                label=f'{self._get_zone_korean(zone_name)} 구간'
            )
            ax.add_patch(rect)
